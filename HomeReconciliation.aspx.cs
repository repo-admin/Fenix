...
namespace Fenix
{
	public partial class HomeReconciliation : BasePage
	{
		#region Properties
		...
		private void fillPagerData(int pageNo)
		{
			...
			pagerData.ColumnList = " [ID],[MessageId],[MessageTypeId],[MessageDescription],[MessageDateOfReceipt],[CommunicationMessagesSentId],[ItemSupplierId],[ItemSupplierDescription],[Reconciliation],[MessageDateOfShipment],[ItemDateOfDelivery],HeliosObj";
			pagerData.WhereClause = "Reconciliation = 0";
		...
		protected void grdData_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			try
			{
				string proS = string.Format("SELECT [ID] ,[CMSOId] ,[ItemID] ,[ItemDescription] ,[ItemQuantity]  ,[ItemUnitOfMeasure] ,[ItemQualityId] ,[IsActive] ,[ModifyDate] ,[ModifyUserId],[GroupGoods],[Code],[DescriptionCz],CMRSIItemQuantity " +
		...
		protected void gvConfirmationItems_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			GridViewRow selectedRow = gvConfirmationItems.SelectedRow;

			string proS = string.Empty;
			try
			{
				proS = string.Format("SELECT ID,[cdlStocksName],[ItemVerKitDescription],[GroupGoods],[Code] ,[DescriptionCz],[ItemOrKitQuantity],[ItemOrKitFree],[ItemOrKitUnConsilliation] ,[ItemOrKitReserved] ,[ItemOrKitReleasedForExpedition] ,PC" +
						  " FROM [dbo].[vwCardStockItems] WHERE [IsActive] = {0} AND ItemOrKitId = {1}", 1, selectedRow.Cells[2].Text);
			...
			try
			{
				proS = string.Format("SELECT [ID] ,[CMSOId] ,[ItemID] ,[ItemDescription] ,[ItemQuantity]  ,[ItemUnitOfMeasure] ,[ItemQualityId] ,[IsActive] ,[ModifyDate] ,[ModifyUserId],[GroupGoods],[Code],[DescriptionCz] " +
						  " FROM [dbo].[vwReceptionConfirmationIt] WHERE [IsActive] = {0} AND ItemID = {1}", 1, selectedRow.Cells[2].Text);
		...
		protected void OrderView(int id, int idC)
		{
			try
			{
				...
				proS = string.Format("SELECT [ID] ,[CMSOId] ,[HeliosOrderId] ,[HeliosOrderRecordId] ,[ItemId] ,[GroupGoods] ,[ItemCode] ,[ItemDescription] ,[ItemQuantity] ,[ItemQuantityDelivered]" +
									 ...
									 " FROM [dbo].[CommunicationMessagesReceptionSentItems] WHERE CMSOId={0}", id);
		...
		/// <summary>
		/// Pro ověření funkčnosti EF 6
		/// </summary>
		private void debug()
		{
			if (Session["Logistika_ZiCyZ"].ToString() == BC.REZLER && BC.IsDebug == false)
			{
				tbxMessageIdFlt.Text = "";
				using (var db = new FenixEntities())
				{
					try
					{
						...
						var cardStockItems = from c in db.CardStockItems												
											select c;
						tbxMessageIdFlt.Text = string.Format("CardStock count:{0}", cardStockItems.Count().ToString());
					}
					catch (Exception ex)
					{
						BC.ProcessException(ex, AppLog.GetMethodName());
